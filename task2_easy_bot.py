...
def play():
    print("Игра будет происходить против бота\n")
    player_1 = input("Имя первого игрока: ")
    player_2 = "Терминатор Т-800"
    ...
    while value_candys > 27:
        if turn:
            ...
            bot_turn= randint(1,29)
            value_candys -= bot_turn
            print(f'Терминатор Т-800 уничтожил {bot_turn} конфет')
            turn = True
            print(f"На столе осталось: {value_candys} конфет")
